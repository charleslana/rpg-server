...
export class SocketService {
  ...
  private handleDisconnect(socket: Socket, server: Server): void {
    socket.on('disconnect', async () => {
      ...
      if (existRoom) {
        await socket.leave(existRoom.id);
        const user = existRoom.users.find((user) => user.id === clientId);
        // TODO user.originalId refatorar
  ...
  private handleCreateRoom(socket: Socket, server: Server): void {
    socket.on('createRoom', async (rooName: string) => {
  ...
  private handleLeaveRoom(socket: Socket, server: Server): void {
    socket.on(
      'leaveRoom',
      async (roomId: string, userName: string, originalId: number) => {
  ...
  private changeRoomOwner(
    ...
    if (isRoomOwner && userLast) {
      ...
      // TODO userLast.originalId refatorar
      this.socketRoomService.updateRoomOwnerId(roomId, userLast.originalId!);
    }
  }

  // TODO refatorar getRoom
  private getRoom(server: Server, roomId: string) {
  ...
  private handleChangeUserWatch(socket: Socket, server: Server): void {
    socket.on(
      'changeWatch',
      (roomId: string, watch: boolean, originalId: number) => {
        const isRoomOwner = this.socketRoomService.isRoomOwner(
          roomId,
          originalId,
        );
        if (!isRoomOwner) {
          this.socketRoomService.updateUserWatchInRoom(
            roomId,
            socket.id,
            watch,
          );
